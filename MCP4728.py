...
class MCP4728:
    ...
    def set_channel_value(self, channel: int, value: int, vref: int = 0, gain: int = 0, pd: int = 0, udac: int = 0) -> None:

        # high_byte = (channel << 4) | (value >> 8)
        # low_byte = value & 0xFF   # Lower bits
        # command = [high_byte, low_byte]  # getting rid of this
        command = bytearray(3)
        command[0] = (_MCP4728_MULTI_WRITE_CMD | (channel << 1?) | udac?)
        command[1] = ((value >> 8) & 0x0F)
